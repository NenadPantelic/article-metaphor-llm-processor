...
def get_contents_of_all_files(dir_path):
    file_contents = {}
    for root, dirs, files in os.walk(dir_path):
        for file in files:
            ...
            with open(filepath, "r", encoding='utf-8') as fin:
                ...
                file_contents[file_name] = chunks
...
if __name__ == "__main__":
    ...
    results = {}
    files_contents = get_contents_of_all_files("./test-data")

    for fn, chunks in files_contents.items():
        document_id = str(uuid.uuid4())
        print("Working on document ", document_id)
        segments = []
        analyzed_text = AnalyzedText([], utc_now())
        doc_text = ""
        for idx, chunk in enumerate(chunks):
            ...
            doc_text += chunk
            at = pipeline_processor.execute_pipeline(data)
            analyzed_text.segments.extend(at.segments)
